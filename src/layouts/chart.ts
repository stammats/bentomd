...
function renderBarChart(items: ChartDataItem[], colors: string[]): string {
  ...
  const bars = items
    .map((item, i) => {
      ...
      return (
        ...
        `font-size="14" fill="var(--color-text, #0f172a)">${escapeHtml(item.label)}</text>` +
        `<rect x="${labelWidth}" y="${y}" width="${w}" height="${barHeight}" rx="4" ` +
        `fill="${color}" />` +
        `<text x="${labelWidth + w + 8}" y="${y + barHeight / 2 + 5}" ` +
        `font-size="13" fill="var(--color-muted, #64748b)">${item.value}</text>`
      );
    })
    .join('\n');

  return `<svg class="chart-svg" viewBox="0 0 ${chartWidth} ${svgHeight}" xmlns="http://www.w3.org/2000/svg">${bars}</svg>`;
}

function renderPieChart(items: ChartDataItem[], colors: string[]): string {
  ...
  const legend = items
    .map((item, i) => {
      ...
      return (
        ...
        `<text x="372" y="${ly + 12}" font-size="13" fill="var(--color-text, #0f172a)">${escapeHtml(item.label)} (${item.value})</text>`
      );
    })
    .join('\n');
  ...
  return (
    `<svg class="chart-svg" viewBox="0 0 560 ${svgHeight}" xmlns="http://www.w3.org/2000/svg">` +
    `${segments}${center}${legend}</svg>`
  );
}

function renderLineChart(items: ChartDataItem[], colors: string[]): string {
  const width = 800;
  const height = 400;
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    gridLines.push(
      `<line x1="${pad.left}" y1="${y}" x2="${width - pad.right}" y2="${y}" stroke="var(--color-muted, #64748b)" stroke-opacity="0.15" />` +
      `<text x="${pad.left - 8}" y="${y + 4}" text-anchor="end" font-size="11" fill="var(--color-muted, #64748b)">${val}</text>`,
    );
  }
  ...
  const xLabels = points
    .map(
      (p) =>
        `<text x="${p.x}" y="${height - 10}" text-anchor="middle" font-size="11" fill="var(--color-muted, #64748b)">${escapeHtml(p.item.label)}</text>`,
    )
    .join('\n');
...
export const chartRenderer: LayoutRenderer = (slide, _config) => {
  ...
  const inner = `<div class="chart-container">${titleHtml}${chartSvg}</div>`;

  return wrapSlide('chart', inner);
};
